     ...
     public class Identify
    {
                        ...
                        if (buff >= 0)
                        {
                            ...
                            if (identifiers != null)
                            {
                                for (int i = 1; i < identifiers.Length; i++)
                                {
                                    ...
                                    {
                                        Gui.WL_UserAction("User was logged out", true);
                                        LoggedInID = buff;
                                        SetNameOfLogedInUserPrivate();
                                        FormLoginIsShown = false;
                                        Gui.WL_UserAction("User was logged in",true);
                                        return;
                                    }
                                }
        ...
        {            
            try
            {
                for (int i = 1; i < 31; i++)
                {                    
                    if (ID == Convert.ToInt64(XML_handler.settingsXML.Element("root").Element("USERS").Element("User" + i).Element("ID").Value))
                    {
                        return XML_handler.settingsXML.Element("root").Element("USERS").Element("User" + i).Element("Name").Value;
                    }
                }
        ...
        public string GetName(int index)
        {
            try
            {
                string a = settingsXML.Element("root").Element("USERS").Element("User" + index).Element("Name").Value.Replace("\"", "");                
                return a;
            }
            catch (Exception e)
            {
        ...
        public bool GetPermision(int permissionNum)
        {
            ...
            // 9  - 
            // 10 - 
            var Collection = IDs(settingsXML);            
            for (int i = 1; i < Collection.Length; i++)
            {
                if (Collection[i] == LoggedInID)
                {
                    try
                    {
                        string a = settingsXML.Element("root").Element("USERS").Element("User" + i).Element("permission"+ permissionNum).Value.Replace("\"", "");
        ...
        {            
            ...
            Gui.WL_UserAction("Unauthorised access was denied. User ID: " + LoggedInID + " - " + LogedInUserName, true);
        }
        ...
        private void SetNameOfLogedInUserPrivate()
        {
            ...
            try
            {
                for (int i = 1; i < identifiers.Length; i++)
                {
                    tmp = Convert.ToInt64(settingsXML.Element("root").Element("USERS").Element("User" + i).Element("ID").Value.Replace("\"", ""));
                    if (tmp == LoggedInID)
                    {
